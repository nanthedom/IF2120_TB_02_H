...
void CreateProfil(Profil *P){
    Word bio, noHP, weton; MatrixFoto M;
    CreateWord(&bio);
    CreateWord(&noHP);
    CreateWord(&weton);
    CreateMatrixFoto(&M);
...
void ReadFoto(Profil *P){
    ...
    while(i < 5){
        while(j < 5){
            ...
            Warna(F) = format.TabWord[k]; k++;
            while(format.TabWord[k] == BLANK){
                k++;
            }
            Karakter(F) = format.TabWord[k]; k++;
            PIXEL(M,i,j) = F;
            j++;
        }
        if(i < 4){
